---
name: help
description: 'Analyzes current state and user query to answer Framework questions or recommend the next skill(s) to use. Use when user asks for help,Framework help,what to do next,or what to start with in Framework.'
---
...
Help the user understand where they are in their Framework workflow and what to do next,and also answer broader questions when asked that could be augmented with remote sources such as module documentation sources.
...
- **Catalog**: `{project-root}/_framework/_config/help.csv` — assembled manifest of all installed module skills
- **Config**: `config.yaml` and `user-config.yaml` files in `{project-root}/_framework/` and its subfolders — resolve `output-location` variables,provide `communication_language` and `project_knowledge`
...
- Skill name in backticks — e.g.,`create-prd`
