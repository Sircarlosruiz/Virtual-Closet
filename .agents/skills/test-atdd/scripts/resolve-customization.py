...
# ///
"""Resolve customization for a Framework skill using three-layer TOML merge.

Reads customization from three layers (highest priority first):
 1. {project-root}/_framework/customizations/{name}.user.toml (personal,gitignored)
 2. {project-root}/_framework/customizations/{name}.toml    (team/org,committed)
...
def find_project_root(start: Path) -> Path | None:
  """Walk up from *start* looking for a directory containing ``_framework/`` or ``.git``."""
  current = start.resolve()
  while True:
    if (current / "_framework").is_dir() or (current / ".git").exists():
      return current
...
def main() -> None:
  parser = argparse.ArgumentParser(
    description="Resolve Framework skill customization (three-layer TOML merge).",
    epilog=(
  ...
  parser.add_argument(
    "skill_name",
    help="Skill identifier (e.g. agent-pm,product-brief)",
  )
  parser.add_argument(
    "--key",
    action="append",
  ...
  if project_root is not None:
    customizations_dir = project_root / "_framework" / "customizations"
