...
├── SÍ → Leerlo completo desde .agents/reports/[nombre].md
└── NO → Listar todos los archivos en .agents/reports/ con su estado
...
> *"Esta es la sección de auditoría. ¿La agrego al final de `.agents/reports/[nombre-del-plan].md`?"*

Esperar confirmación. Si el usuario aprueba, usar `ask` para modificar el archivo e inyectar la sección al final del report.

Confirmar con:
> *"Auditoría persistida en `.agents/reports/[nombre-del-plan].md`. El estado del plan no cambia — la auditoría es una capa de análisis sobre el trabajo ya realizado."*
...
Agente de Planificación → genera el plan
         ↓
Agente de Ejecución → implementa el plan → actualiza .agents/reports/
         ↓
Agente Auditor → analiza lo ejecutado → inyecta sección en .agents/reports/
         ↓
